...
- **S3. Rules in code.** `turnstile_code`, first because the thin application binds it.
- **S4. The example.** `turnstile_example` and `example_code`, the seam enforcing from the first commit; the role table, the override, re-authentication, `review`, and the count test. First real run, ledger mode none.
...
- **S6. History.** `example_code` in ledger mode Ecto: fact fields through the macro, the genesis migration, the history scenarios; mode none stays a second Tier 2 job.
- **S7. Postgres.** Adapter package, then thin application.
- **S8. Point-in-time review**, drift scenarios, replay for code and Postgres.
