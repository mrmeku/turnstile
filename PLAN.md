...
6. **Rename the packages and place the contract.** `turnstile_core` becomes `turnstile`, and `turnstile_example` becomes `example`. Move its modules into `core/` and `adapter/`, write the boundary declarations, extend `StructureTest` with the effect rule, and write the classifier property. Two more things belong here, because §1 names them and no other step reaches them: the fake adapter is test support rather than an adapter of the contract, so it moves to `Turnstile.Test.Fake`, and the two Credo checks become `turnstile_credo`, the package §1 lists, which every app takes as a development and test dependency so a run from any directory finds the checks.
