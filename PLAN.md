...
6. Can my compliance team, and the 3PAO, read what the engineers built? (the SSP and the SAR)

## Words

Three vocabularies, at three speeds, each kept where its speed is harmless.

The port's words, **subject, object, operation, environment condition**, are NIST SP 800-162's. They do not move, and they are the only vocabulary core, the events, and the adapters speak, besides Ecto's query language. Control identifiers such as AC-3 and AU-2 are SP 800-53's: a control keeps its number for life, withdrawn ones are never reused, revisions add. Scenarios cite them. FedRAMP's program words (SSP, control implementation statement, origination, POA&M, CRM, 3PAO, authorization boundary, Key Security Indicator) change on a cadence of months and their identifiers are being restandardized. Nothing in the library speaks them. The deferred generator (`docs/assess.md`) would, reading them from pinned releases of the catalog and the profile, so that a revision is a dependency bump, not a remodel.
...
The ledger is a behaviour in core: append a fact event, read fact events in order from a position, report the current position. Current state is a fold over it; state at a time is the fold stopped there. **Replay** is state at a time plus the policy version in force then, and it reproduces any decision, which no adapter can do alone: Elixir modules keep nothing, a database keeps nothing without temporal tables, and a policy engine versions its rules but not the attributes it was sent. Decisions are reads, so they are a second stream beside the ledger, each stamped with the position it saw.

Positions come from a counter row. Every fact-writing transaction locks one row in a library-owned counter table, takes the next positions from it, and inserts its events with them, so commit order is position order, there are no gaps, and a reader is "from position N". The head is the counter row's committed value, every position at or below it is committed, and replay to the head is exact. The lock serializes fact-writing transactions; that cost is measured by the suite rather than argued about. The same mechanism runs on every database, and the dialect seam stays so a Postgres watermark reader can be added later. The row is named in the configuration, so a test can take its own and async tests never contend.

A **projection** is how an adapter's working state relates to the ledger. For RBAC, Postgres, and Cerbos the state is the application's tables, so the projection is identity and the applied position is the head. For a relationship graph the state is a copy in the engine's own store, kept current by a **projector** that drains the ledger from a checkpoint, advances the checkpoint after every acknowledged write, and writes only the difference between what the fold requires and what the engine holds; it can be rebuilt from position zero into a fresh store beside the serving one, and the swap is a dated configuration change. Its lag is a component of revocation latency and its drift is what reconcile compares. An adapter that projects declares that it requires a ledger.

The ledger's shape is the transactional outbox: a write and the record of it commit in one transaction, and consumers read the record afterwards from the same database, so nothing is published that did not commit and nothing commits unpublished (Richardson, https://microservices.io/patterns/data/transactional-outbox.html). Three readers consume it. The OpenFGA projector needs the old and the new value of every fact and gapless positions, because it drains by diff and checkpoints by position. Replay folds the events to a position. Reconcile folds them to the head and compares the fold with the tables. The one departure from the pattern as usually drawn is that this outbox is never emptied: the ledger is the system of record and stays complete for the life of the system.

Two modes, chosen by the application; a third, the application's own event store as the ledger, is deferred and listed at the end:
...
- **None.** The tables are the only truth. Revocation latency is still measured; history, drift, and automated account audit are the application's to provide; an adapter that requires a ledger cannot be bound, and boot fails with `Turnstile.Error.Unsupported`.
...
Which decision applies to which query is a rule, not a guess. A schema is protected iff it declares an object type. The seam judges a query by its root source: a decision naming a different object type is refused; a preload or association query needs its own decision unless the parent schema declares the association as a carried relation; in a multi-source query every other protected source must be carried by the root. Lookup tables declare no object type and pass without a decision.

Exemptions are per call, with a reason, and are recorded with their reasons. A library-internal kind covers the writes the library makes itself (the ledger's tables, the projector's checkpoint, genesis), and only library callers may claim it. Reconcile, genesis, and test truncation run through a second Repo module the application declares with the owner role, which the seam treats as wholly library-exempt.

The seam's shape is proven, not assumed. `Turnstile.Repo.Surface` is a plain list that classifies every Repo function: mediated through the query hook, overridden, wrapped to demand an exemption, or plumbing that touches no rows. The overrides are defined in a `@before_compile` hook with `defoverridable` and `super`, so an application's own query hook runs inside ours, and the macro checks that `use Turnstile.Repo` follows `use Ecto.Repo`. One Tier 1 test, shipped as `Turnstile.Conformance.RepoCase` so the thin applications and adopters run it against their own Repo, diffs the list against the Repo's exported functions, a subset being acceptable for a `read_only` Repo, and calls each non-plumbing function without a decision, expecting `Turnstile.Error.Unmediated`; an Ecto release that adds a function fails that test by name (`docs/reference.md` §6). An adapter may wrap every mediated call and write through `around_query/3`, the seam's fourth extension point; the Postgres adapter uses it to open a transaction where none is open and to set the subject's session settings on every call, so two subjects in one transaction each set their own. What the seam cannot see (SQL that bypasses the Repo module, a second Repo without the extension) two shipped Credo checks flag, advisory only; the claim stays with the seam. Bulk writes to fact fields go through a declared API that skips rows whose value did not change, so a million-row sync that changes nine facts records nine (`docs/reference.md` §8). For a single-row update or delete the seam re-reads the row under the dialect's lock and takes the old value from the re-read, not from the changeset; an upsert on a fact schema is refused with a pointer to the bulk API.
...
Four, three named for where the rules live and one for its model, because where rules live, not the access-control model, is what differs in enforcement, in who may change a rule, and in what each costs inside an authorization boundary, where every engine is an inventory item with its own scanning, hardening, and controls to answer for.

- **RBAC in code**, `turnstile_rbac`. Elixir modules; a deploy is a policy version; enforcement is the seam. First, because RBAC is what the baseline expects: AC-2(7) offers a role-based or an attribute-based scheme, AC-3(7) is the catalog's RBAC enhancement, and what fails assessment is a *single* role, which cannot satisfy AC-5 or the AC-6 family. Its role→permission table is declared data, enumerable for review; attribute predicates sit beside it for the rules roles cannot express.
...
- **OpenFGA**, `turnstile_fga`. A relationship graph: facts become tuples, rules become an immutable, versioned model, and a projector drains the ledger into the engine's own store, so the adapter requires a ledger. The adapter talks to the server through a client behaviour with an in-memory fake, built first, so the projector's convergence and drift cases run without a server; the projection cases run on the committed repo and drive the drain synchronously. Explanations are paths; implied controls and separation of duties are the cleanest of any adapter; `scope` is a capped `ListObjects`, declared limited; there are no write gates. It costs a server and a datastore inside the boundary (two inventory items, one engine if the datastore shares the application's Postgres instance) and a drain interval inside the revocation clock.

RBAC and Postgres are the recommended pair; Cerbos is for teams whose rules are owned by people who do not write Elixir; OpenFGA is for domains with real hierarchy, or organizations that have standardized on it. The example is one library application bound once per adapter by a thin application, and the four thin applications sit side by side in the README so that a team can choose from the evidence, not from the marketing: what each adapter enforces without help, what each explains, what each costs, and where each records a gap. The graph adapter's design note is `docs/reference.md` §13.

Admission for any adapter: self-hosted, inside the boundary; no facts in tokens, because the identity provider contributes the subject and nothing else, and a role in a token is frozen until expiry; deny on failure. An adapter package declares only what is true of it in any domain: whether it requires a ledger and its `scope` cap. The capability declaration per rule, native, limited, or unsupported for each of C1 to C13 with the enforcing component named, lives in the thin application, because it is a claim about this domain on this adapter. **Revocation latency** is evidence, not a declaration: a Tier 1 case per adapter on the committed repo measures the time from a revoking write's commit to the first denied check, decomposed into commit, projector drain, policy propagation, replica lag, and cache, with components the test environment lacks printed "not measured"; the number is recorded for comparison with the FedRAMP-assigned PS-4 value ⟨verify⟩, and no test asserts it. Every adapter's decision half reaches the database only through the ledger behaviour and `around_query/3`; its `scope` half needs Ecto. Each adapter package ships `priv/conformance/`, the artifacts Tier 1's neutral fixture needs on that mechanism, so the fixture stays core's and the CUI domain stays out of the adapters. Comparison and notes: `docs/reference.md` §4.

## The line

Opinionated below the port, neutral above it.

Below: Ecto is required for the seam and for `turnstile_ledger`, without apology: a seam is the only way a library proves "always invoked," and a ledger the only way it records history. Postgres is the reference database and the only one the suite runs; the ledger's few database-dependent mechanisms live behind a dialect behaviour, `Postgres` shipped and no more; an adopter on another database implements the behaviour and proves it with the ledger's conformance case; the RLS adapter is Postgres.

Above: `authorize`, `check`, `batch`, `explain`, the events, the ledger contract, policy versions, and `review` over a population reach the database only through the ledger behaviour and `around_query/3`, and a compile-time `boundary` rule keeps `authorize`, `check`, `batch`, `explain`, and `review` off `ecto_sql`.

Two altitude tests; if either needs a change in core, core is too low. *Bring your own adapter*: implement the adapter behaviour, ship a declaration, pass Tier 1. *Bring your own store, or none*: an event-sourced application on Cerbos or OpenFGA with no Ecto runs the port, forgoing `scope` and the seam, with mediation, drift, and point-in-time review as its own claims. The graph adapter's projector drains that application's event store as readily as the Ecto ledger; nothing below the port is required of it. The second test stays a constraint core is designed to, and the event-store mode that would exercise it is deferred.
...
Document, Portion, Program, Assignment, OfficeRole, Marking, and the marking proposal declare object types and are protected. Agency, Office, and Category declare object types too, because a rule reads a column of theirs (the agency's nationality, the office's agency, the category's implied controls) and the coverage case requires every column a rule reads to be a declared fact or a reference to an object; a lookup table no rule reads declares none and passes the seam without a decision. A schema a protected schema carries may carry further: the closure counts, for the seam's admission and for coverage alike. The fact mapping is the domain's own: Assignment and OfficeRole rows are relationships with a role attribute, and an OfficeRole row is an object of its own as well, carrying its account, its office, and its role, because an account may hold both roles in one office while a fold keys a relationship by its subject and its object alone; a Marking's controls and list are set-valued and emit one event per element; employment and nationality are subject attributes; a Document's marking and decontrol and a Portion's marking are object attributes. The domain, its contexts, its web layer, and every scenario live in `turnstile_example`, a library application; `example_rbac`, `example_postgres`, `example_cerbos`, and `example_fga` each bind one adapter to it and add only what that adapter needs.

## Evidence

A **scenario** is a test whose name is a sentence, citing a control in the Moderate baseline (marked when it cites beyond the baseline). Scenarios every adapter passes identically are kept, because a buyer checks them off; parameters a control leaves open are configuration the scenario reads, never constants in a rule. Groups follow what a 3PAO examines: enforcement, least privilege, separation of duties, revocation and expiry, decision audit, access review, re-authentication, emergency override, change control on policy (ids: `docs/reference.md` §1). The scenario table (id, sentence, group, controls cited, the rule it tests) is written before code and frozen with the contracts.

Two tiers. **Tier 1**, in core, proves the port's guarantees, the seam, and the ledger over a neutral fixture, on Postgres and nothing else, with shape tests (query, record, and row counts, stated per adapter and per ledger mode) checked but not cited, and three projection cases (measured lag, drift from an out-of-band write, convergence of a re-drain) that activate only for an adapter that declares a projection and run on the committed repo; any adapter that passes is conformant with no example work. **Tier 2** is the CUI scenarios, run once per thin application. Each thin application's suite ends with a count test: the scenarios defined without a skip equal the table's rows minus the declared `unsupported` rules and, in mode none, the ledger-marked ones, and are more than zero: a suite that passes by skipping proves nothing.

`mix turnstile.review` prints who can do what, today or, with a ledger, on a date. The generator that would fold declarations, results, and configuration into control implementation statements is deferred; its design is `docs/assess.md`.

The library does not claim authorization status, own identity, own the audit store, or promise portability it has not tested.
...
turnstile/
  ...
  docs/        reference.md · testing.md · code.md · delivery.md · writing.md · assess.md · glossary-index.md
  apps/
    ...
    turnstile_ledger/     the Ecto ledger: counter row, events table, migration and genesis helpers,
                          the catalog check, bulk fact API, the dialect behaviour and its Postgres
                          implementation, reconcile, replay, mix turnstile.review and
                          mix turnstile.schema_dump; ecto_sql
    turnstile_rbac/       RBAC in code: declared roles and permissions, attribute predicates;
                          priv/conformance/
    ...
    example_rbac/         the thin apps, each with its own config and Application.start/2: the adapter
...
- **S2. Core mechanisms.** The seam with its surface list, `RepoCase`, and the upsert refusal; the fake adapter, the in-memory ledger, Tier 1 with the latency case template.
- **S3. RBAC in code.** `turnstile_rbac`, first because the thin application binds it.
- **S4. The example.** `turnstile_example` and `example_rbac`, the seam enforcing from the first commit; the role table, the override, re-authentication, `review`, and the count test. First real run, ledger mode none.
- **S5. The ledger.** One shard around the counter row: table, helpers, genesis, the bulk API, the dialect behaviour and its Postgres implementation, the reader, reconcile, replay, the catalog check, the interleaving and lock-cost cases, `mix turnstile.review`.
- **S6. History.** `example_rbac` in ledger mode Ecto: fact fields through the macro, the genesis migration, the history scenarios; mode none stays a second Tier 2 job.
- **S7. Postgres.** Adapter package, then thin application.
- **S8. Point-in-time review**, drift scenarios, replay for RBAC and Postgres.
- **S9. The README** with the comparison table, the schema dumps, the workflow.
- **S10. Cerbos.** Adapter package with its replay and drift scenarios, then thin application.
- **S11. OpenFGA.** The client behaviour and its fake, then the adapter with its replay and the throwaway graph, then the thin application.

## Decisions, each with the alternative it rejected

- FedRAMP on two tracks over one catalog, 800-53 ids as the key; not Rev 5 alone. Scenarios cite the ids; each track's own words belong to the deferred generator.
- The Moderate baseline, because CUI needs it; CUI over the classified domain.
- RBAC in code first, because RBAC is the baseline's default and a single role is not RBAC; not RBAC as the lesser adapter.
- Four adapters, named for the mechanism, OpenFGA included as the graph with its boundary cost stated; not leaving it out on that cost, because the example is where a team compares the four and the cost is evidence, not a reason to hide the option.
...
- Mediation at the Repo seam, at runtime: a schema protected by declaring an object type and matched by its root source, exemptions per call with a reason, the surface a plain list that a shipped test diffs against the Repo's exports, the overrides in `@before_compile` over `defoverridable` and `super`, a fourth extension point for the adapter; not a compile-time heuristic that could not see writes two calls down, and not an after-compile hook that failed the build on an unclassified function, because the shipped test proves the same list at runtime, against the adopter's own Repo, with no compiler machinery to maintain.
...
- One library application and four thin applications, each proven by the same scenarios on every pull request, the adoption guide deferred; not the example built by migration with tags as the guide and a CI diff as the guard, because every encoding of the steps rotted or forced a rebase cascade.
- The generator deferred to its own document, `docs/assess.md`; not built beside the runtime, because it reads what the runtime records and the runtime needs nothing from it.
- Cerbos and OpenFGA after the README; not in the middle, because the recommended pair proves the port, the seam, and the ledger first, and the other two add engines to a design that already runs.
...
- Capability declarations per rule in the thin application, three levels with the enforcing component named, only unsupported allowed to skip a scenario; not in the adapter package, which would carry the domain, and not a fourth level for a cell that was never a rule.
...
- The reasons for each decision in this file, the conformance mechanisms in core; not a records directory.
...
Each returns as its own stage after S11, when its condition holds.

- The assessment generator, `turnstile_assess`: the package that folds declarations, results, and configuration into control implementation statements, with the OSCAL, 20x, and `--diff` outputs behind it. Its design is `docs/assess.md`; it returns once the four thin applications run and have evidence for it to read.
