...
- The ledger projects into the engine: a checkpoint per acknowledged write, a drain by diff, rebuild into a fresh store, a client behaviour with a fake ⟨D23⟩ ⟨D24⟩ ⟨D25⟩ ⟨D37⟩; not engine-owned facts with an outbox, because the ledger is the assessment's system of record.
...
- Decision records in `docs/decisions.md` ⟨D41⟩, the conformance mechanisms in core and the generator ⟨D40⟩; not a records directory.
