...
| Two deciders rescue database driver errors for a driver they do not carry | Remove the rescue. Core fails closed on any exception from a decider. | 7, 8 |
...
6. **Rename the packages and place the contract.** Done. `turnstile_core` is `turnstile` and `turnstile_example` is `example`. The contract's modules sit in the three places §2 names: the root keeps the public contract, `core/` holds the surface classifier, the matching, the source, the mediation, and the configuration schema, and `adapter/` holds the seam, the caller, the option, and the overrides, which writes calls into a module being compiled and so sits beside the seam rather than beside what decides. `StructureTest` gained three rules over every package: a module in `core/` makes no call to the outside world, a module in `core/` names no module in `adapter/`, and a module in `core/` or `adapter/` carries `@moduledoc false`. Each was tried against a planted module before it was trusted. The classifier property generates names and arities on and off the surface and holds each to one bucket or none. The fake adapter is `Turnstile.Test.Fake`, and the two checks are `turnstile_credo`, which every app takes as a development and test dependency, so a run from any directory finds them. One module keeps its own place: `Turnstile.Conformance.RepoCase.Fixture`, because `Turnstile.Conformance` is a boundary of its own, and a fixture in `Turnstile`'s `core/` would cross that boundary in both directions.
7. **`turnstile_rbac`.** Done. The root keeps what an adopter calls: the adapter, the binding, the policy macro with its three structs, the coverage check, and the version. `core/` holds what decides, the rule and the clause builder, and `adapter/` holds what touches the world, the query the decider runs and the appending of a version to the ledger. The version is divided at that line: what a version is stays public, and publishing it is the adapter's. The rescue for the two driver errors is gone, because the port asks the adapter through one place and turns any exception a decider raises into the engine error that denies, with the adapter and the callback in the detail and the exception on the event. A batch and a filter record the reason one object records rather than the default denial, which is what carrying that answer beside the verdicts buys.
8. **`turnstile_postgres` and `turnstile_cerbos`.** Done. Both packages take the three places on the rule step 7 settled. In `turnstile_postgres` the root keeps what an adopter calls, the adapter, the binding, the catalog, a policy, the migration helpers, the coverage check, and the version; `core/` holds what decides, the identifier check, the session settings, and what a policy expression declares; and `adapter/` holds what touches the world, the session, the statement the decider runs, and the appending of a version to the ledger. In `turnstile_cerbos` the root keeps the adapter, the binding, the declarations, the client, the coverage check, the decisions reader, the propagation measurement, the request bodies, and the version; `core/` holds the plan compiler and the codec; and `adapter/` holds the values read through the repo, the calls to the sidecar, the appending of a version, and the wait. The boundary of each package exports what the root keeps and nothing else. The rescue for the named driver errors is gone from both deciders, and neither package names an error of a driver it does not carry; the rescue that stays is the catalog read of `Turnstile.Postgres`, which is a callback answering an engine error rather than a decider raising. `Turnstile.Cerbos` no longer depends on the test kit: `Turnstile.Cerbos.Adapter.Wait` asks on a monotonic clock, so an adjustment of the system clock cannot move the deadline, and reports the interval between asks as the floor of any measurement. The codec property generates a type and a value of it, over strings, integers, floats, booleans, dates, times, naive and UTC moments, and lists of each, and holds `Ecto.Type.cast/2` of the encoding to the value that was encoded, beside the JSON the sidecar is sent. It found an array a policy could not read: a list crossed whole, so an array column of dates reached the sidecar as structs JSON cannot carry, and a list now crosses as the list of its values encoded. Two conventions came out of the work and hold umbrella-wide: an interior module states that it is hidden before it says what it is, and a type a public module's spec names lives on a public module.
9. **`turnstile_relay`.** Done. The package takes the three places on the rule step 7 settled: the root keeps what an adopter calls, the supervisor with `wake/1`, `drain_once/1`, and `options_schema/0`, the job behaviour, the entry, the pass, the cursor, the migration helper, and `JobCase`; `core/` holds what decides, the pair the advisory lock is taken on and the arithmetic that says when the next pass runs; and `adapter/` holds what touches the world, the lock, the pass, and the worker with its timer. It carries no authorization concept, so its `lib` takes none of the library's other packages, and the cursor and the lock reach a repo as a library caller, which a seam repo in the owner's role grants and a plain Ecto repo ignores. A pass is one transaction: ask for the transaction-scoped advisory lock, read the cursor, read a batch above it through the job, deliver it, advance the cursor, commit. A pass that does not commit leaves the cursor where it was, so delivery is at least once in position order, and the durability property holds that over a job that delivers and then fails at the points `StreamData` picks. `JobCase` holds a job to the three things a runner relies on, over a population module of the job's own, so the template names no table. The clock is an option and the moment on a pass is read from it once the transaction has returned, which closes the half of the wall-clock defect of §10 that falls here; the other defect of §10 that falls here is one line, `:inets` in the `extra_applications` of `turnstile_fga`, beside the `httpc` its client calls. The package has no consumer yet, so it is proved by its own tests, and step 10 gives it one.
10. **`turnstile_fga`.** Done. The store is kept in step by markers rather than by a fold of history: a change writes a marker row, `Turnstile.Fga.Outbox` is a `Turnstile.Relay.Job` over that table, and a pass reads the markers above the cursor, gathers the objects they name, and brings each into step by reading what the tables require, reading what the store holds, and writing the difference. Delivering a marker twice costs a read and no write, which is what lets the relay's at-least-once delivery stand as correctness here. `Turnstile.Fga.Core.Drain` holds the arithmetic of a difference and is proved by a property over generated held and required sets; the tuple encoding is `Turnstile.Fga.Core.Codec`, proved by a property that holds a condition's values to the values that were encoded. Three templates hold an application's binding to what a decision rests on: the mapping it declares, the drain it runs, and the guard it names. `Turnstile.Fga.Drift` and `reconcile/0` are this adapter's own, over the store rather than over a ledger, and the drift scenario is in `Turnstile.Fga.OutboxCase`.

    The ledger goes with it, which is what step 5 deferred. `Turnstile.Ledger`, its fold, its memory adapter, `Turnstile.FactEvent`, `Turnstile.Facts`, and the `turnstile_ledger` package are deleted, and with them the counter row, the event table, the genesis migration, and the `ledger:` configuration of every thin application. What recorded a version now emits one: each adapter's `adapter/version.ex` writes the artifact where there is one to write and publishes `[:turnstile, <adapter>, :policy_version]`, and nothing is stored, so whoever keeps a record of what was deployed handles that event. A thin application names that event to the scenarios through `Example.Scenarios.Rules.version_event/0`, which is what keeps the change-management bodies free of an adapter's name. The six scenarios the ledger answered are rehomed: `rev-07` reads the grant and the revoke as change events in `Example.Scenarios.Revocation`, and `aud-04`, `aud-06`, `cm-01`, `cm-02`, and `rvw-04` sit in `Example.Scenarios.Audit`. Two are reworded to what the mechanism does: `aud-04` says a marking change emits a change event per row it wrote, the banner's carrying every changed fact field, because the library publishes one event per row and not one per field; and `rvw-04` keeps the half every adapter can answer, that a row written outside the seam emits no change event and the review still reads it from the tables. `Turnstile.Conformance.Scenario` no longer carries `needs_ledger` and `expected_count/1` takes the capabilities alone, so the committed list is one list for every binding. The schema-dump task moved to `turnstile`, and the four committed dumps lost the ledger's tables. The documents that still describe a ledger are step 13's.
11. **`example` and `ExampleWeb`.** Done. The package takes the three places on the rule step 7 settled, with no `adapter/`: the contexts are this package's public surface and reach the database through the seam, so the root holds them, and `core/` holds what decides without touching the world. The web layer is a boundary of its own. `ExampleWeb` holds the router, the two controllers, and the plug that identifies the caller, and `Example` drops `Phoenix` and `Plug` with it, so the package that holds the domain names no web library.

    `core/` took five modules. The queries each context runs are `core/document_query.ex`, `core/account_query.ex`, and `core/review_query.ex`, which build what `Example.Documents`, `Example.Accounts`, and `Example.Review` hand to the repo under a decision or an exemption; a rule is the `dynamic` the port's `scope` answered with and is carried into a query rather than read, so nothing under `core/` decides who may see a row. The mapping to OCSF is `core/ocsf.ex`, hidden because the format a security log takes is the consumer's and not this package's surface, and `Example.Siem.schema_version/0` is what a caller reads the version through. Rule C4 is `core/markings.ex`, whose face is `Example.Banner`, and the property generates markings and holds the rule to its laws rather than to examples: a banner admits no subject a portion of it denies, categories and controls are the union of the portions', a country one portion withholds is released by no banner, the portions' order is not the banner's, a portion counted twice changes nothing, a portion added to a document never widens its banner, and a banner over a banner covers what the inner one covers. Every module under `core/` is at 100% line coverage, which is the number step 12 wires.

    `Example.Controls` keeps the vocabulary alone, the four control kinds and the fields a marking carries. It has to: every schema that holds a marking reads its `Ecto.Enum` values from `Controls.all()` at compile time, and `xref graph --label compile-connected` fails the moment a module a schema compiles against gains a dependency of its own. The vocabulary is the leaf and the arithmetic sits above it, which is also why `Example.Banner` is exported: `Example.Fixture` is a boundary of its own and builds a document's banner the way the domain does.
12. **The thin applications.** Done. Each thin application takes the three places on the same rule the example took them on. `core/` holds what decides and touches nothing: `ExampleRbac.Core.Predicates` is the predicates and the hop filter the policy hands the adapter, `ExampleCerbos.Core.Controls` derives the controls in force at a moment from the rows a query already read, and `ExampleFga.Core.Tuples` states what a row means as tuples. `ExamplePostgres` has no `core/`, because its rules are SQL the migrations carry and nothing in `lib/` decides. What reads the world stays at the root, which is where `ExampleCerbos.Facts` and `ExampleFga.TupleMapping` are: each keeps the half that runs a query and hands the rows to the half that does not.

    The capability declarations are gone, and the tags a scenario carries take their place. A scenario declares its id, its sentence, and the rule it tests; the macro checks all three against the reference's table and tags the test with the id, the rule, and the controls the table cites, so a control id is written in one place and the lint §8 asks for is the table itself. No binding ever declared a scenario unsupported, so nothing was skipped and nothing is lost; what the declarations carried beyond that was prose about mechanism, which is where it now lives, in the "mechanism per rule" table of each thin application's README. `Turnstile.Conformance.Scenarios` keeps `count/0` in place of `expected_count/1`, and the count test reads the ids off the tests the module defined.

    Configuration and the migrations follow. Every thin application runs two migrations, the example's tables and its own binding's rules, and `example_rbac` gained a second that creates nothing, because its rules are a module the adapter evaluates and the database holds none of them: the set is the same shape everywhere, and the dump says what each binding put in the database.

    The two numbers of §5 run. The runtime dependency count of `turnstile` is a test in that package's suite, over the deps of its own Mix project. The line coverage of every `core/` module is `mix test.core` and a CI job of its own, because a partitioned run measures a part of the suite; the run sets an environment variable each package's `test_coverage` reads, and reads the applications it covers off the tree. Ten lines across six packages had been reached by nothing, and each is now tested or gone: gone are the clause that named a mediation's own caller in a refusal, which no refusal reaches, and two option-schema accessors nothing read.
