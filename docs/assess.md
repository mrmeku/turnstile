...
The README's four statements are the four bodies, included by reference, side by side, so a team chooses from the evidence. The thin-app CI job adds, after Tier 2 with the formatter: `mix turnstile.assess`, then `git diff --exit-code statement/statement.md`; `evidence.json` is never diffed. `example_rbac` runs the formatter step twice, once per ledger mode, the mode-none run checked by `--check`. Each thin app's README says where its statement is and how it is regenerated.
...
- Policy versions: which artifact a version is on this adapter, and whether content is by value or by pointer; that replay of RBAC rules depends on the repository's retention.
