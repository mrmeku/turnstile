...
5. registers `ExUnit.after_suite/1` and `System.at_exit/1`, once per VM, to `pg_ctl stop -m immediate` every cluster still running and remove its directory. The suite's end stops its cluster: the umbrella root's `mix test` runs every app's suite in one VM, one after another, and each app's `test_helper.exs` starts its own cluster over the same repo modules, which a cluster still running would hold. The mock clock is defined once per VM for the same reason.
