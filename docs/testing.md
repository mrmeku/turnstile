...
| Configuration: the adapter, ledger mode and counter name, owner repo, seam mode, caps, `adapter_options` | Production reads a `%Turnstile.Config{}` once at boot. In tests, `Turnstile.Test.with_config/1` (for the rest of the process) and `with_config/2` (around a function) put an override in the process dictionary; the seam's resolver checks `self()` and then the `$callers` chain. The override carries the adapter module, which is how `AdapterCase` binds one per test module; the counter name, set by the sandbox setup; and under `adapter_options` the FGA `client` module and `store_id`. `Application.put_env` inside a test is banned; it is global. |
...
| The fake FGA client | `Turnstile.Fga.Client.Fake`, an `Agent` per test via `start_supervised`, holding stores, models, and tuples, named in `adapter_options: [client: ...]` through the override (`docs/reference.md` §13). |
...
- **example_code**, **example_postgres**, **example_cerbos**, **example_fga**: one job per thin app, run in `apps/example_<adapter>/`, in this order:
  ...
  3. `mix turnstile.assess`: the skipped-scenario check, then `statement/statement.md` and `statement/evidence.json` regenerated.
  4. `git diff --exit-code statement/statement.md`; `evidence.json` is never diffed.
  For `example_code`, steps 2 and 3 run twice: with `EXAMPLE_LEDGER=ecto` as above, and with `EXAMPLE_LEDGER=none` writing `TURNSTILE_RESULTS=tmp/results-none.json` and checked by `mix turnstile.assess --check`.
