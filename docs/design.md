...
| `infrastructure/` | What touches the world (a database, an engine, a file, a clock, a process) and what speaks another system's language even when pure: codecs, query builders, wire shapes, identifier rules, session settings, event consumers, repos, migration helpers. | `domain/` | names nothing under `application/`; `@moduledoc false` in published packages |

Calls run one way: the root reaches `application/`, `infrastructure/`, and `domain/`; `application/` reaches `infrastructure/` and `domain/`; `infrastructure/` reaches `domain/`. A nested Boundary owns its own three places under its own directory. `test/support` is not layered and stays flat under the package namespace. Mix tasks stay under `lib/mix/tasks/`. A path names its module. The placement question a contributor asks, in order: Is it named by a consumer of a published package? Root. Does it touch the world, or exist because of how another system works? `infrastructure/`. Does it orchestrate a use case? `application/`. Otherwise `domain/`. Nothing in one package names another package's interior. `boundary` and the structure test in `turnstile_dev` enforce what the compiler can see; `mix xref graph --label compile-connected --fail-above 0` and `--format cycles --fail-above 0` hold the dependency graph flat.
