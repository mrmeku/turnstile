...
**Source.** The two paragraphs above are checked against the Elixir 1.20 changelog at S1 and cited with its URL in the S1 handoff; a claim the changelog does not support is corrected here ⟨D31⟩.
...
| Write a capability declaration as one function clause per record plus a fallback, never a map lookup ⟨D35⟩ | The checker read an empty-map lookup as returning only the default; clauses give it one atom set per rule |
| Type a role definition's actions as `[atom()]`, and every list literal as the checker allows ⟨D35⟩ | The checker cannot narrow a list literal from params; the declared type is the evidence |
...
Two positions, not one: the head at decision time and the position the adapter's state had applied, equal unless the adapter projects, both `nil` in ledger mode none (`docs/reference.md` §7) ⟨D18⟩.

**Structs the reference names**, each defined this way: `%Turnstile.Decision{}`, `%Turnstile.FactEvent{}` (§8), `%Turnstile.PolicyVersion{}` (§5), `%Turnstile.Exemption{kind: :declared | :library}` (§6), `%Turnstile.Subject{kind: :user | :non_person_entity | :privileged}` ⟨D32⟩, `%Turnstile.Config{}` (§15); the errors `Turnstile.Error.Unmediated`, `Turnstile.Error.Engine`, `Turnstile.Error.Unsupported`, `Turnstile.Error.NotAuthorized`, `Turnstile.Error.Invalid`; the behaviours `Turnstile.Adapter`, `Turnstile.Ledger`, `Turnstile.Ledger.Dialect` (seven questions, §11), `Turnstile.Projection` (`checkpoint/1`, `drain_once/1`, `rebuild/1`, `reconcile/1`), `Turnstile.Clock`, `Turnstile.Capabilities` (`capability/1`), `Turnstile.Conformance.Sources`, `Turnstile.Fga.Client`, `Turnstile.Fga.TupleMapping`.
...
|---|---|---|---|
| Elixir version | `mix.exs`, `flake.nix` | `elixir: "~> 1.20.4"`; OTP 29.0.6 pinned by the flake (toolchain-pins §3, verified 2026-09-07) | The complete inference milestone; 1.20 needs OTP 27 to 29 |
| Environment | `flake.nix`, `.envrc`, CI | nixpkgs `nixos-unstable`, locked, provides Elixir, OTP, Postgres 18, and OpenFGA; Cerbos is fetched from its release archive; CI runs `nix develop --command mix quality` | Dev and CI are the same closure; no Docker (`docs/testing.md` §2) ⟨D8⟩ |
...
| Module dependencies | each app's root module | `use Boundary` with explicit `deps:` and `exports:`; the top-layer rule keeps `authorize`, `check`, `batch`, `explain`, and `review` off `ecto_sql` ⟨D18⟩ | Architecture is compile-checked, not reviewed |
...
**Hex pins**, from toolchain-pins §7, verified against hex.pm on 2026-09-07; `mix.lock` is the pin and S0's handoff re-verifies:
...
**Naming** ⟨D4⟩. `Turnstile.` for library modules; `Example.` for `turnstile_example`; `ExampleCode.`, `ExamplePostgres.`, `ExampleCerbos.`, `ExampleFga.` for the thin apps. A struct's module is a noun (`Decision`, `FactEvent`); a behaviour's is a role (`Adapter`, `Ledger`, `Dialect`, `Projection`); an error's is `Turnstile.Error.<What>`; a Credo check's is `Turnstile.Credo.<Rule>`. "Adapter", never "provider". No abbreviations in public names.
...
| `apply/3` and reflection hide call sites from the checker | Confined to four allowlisted modules, each with an enumeration-driven test; the seam's check runs after compilation where it can see everything ⟨D19⟩ |
| An empty-map lookup types as its default alone | Capability declarations are function clauses ⟨D35⟩ |
| A list literal cannot be narrowed from params | Declared list types, `[atom()]` for a role's actions ⟨D35⟩ |
| A stub that raises types every call site as a crash | Fakes return values of the real type ⟨D34⟩ |
| No parametric types, so `Result.t(inner)` cannot be expressed | Concrete `@type`s per use; not worth a workaround |
| Compiler options do not propagate from dependencies | `infer_signatures: true` in every app; documented for adopters in core's README, since the adoption guide is deferred ⟨D42⟩ |
