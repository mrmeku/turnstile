...
*Mode: Reference. What the code must look like outside the ports-and-adapters shape the plan already fixes: how we use Elixir's type system, what is banned, which knobs are turned, and what the CI gate runs. Rules are stated as rules; the reason follows in one sentence where it is not evident. Elixir 1.20.4 on OTP 29.0.6 is the target; 1.20 is the release that completed type inference of every construct, and 1.19 was the step before it.*
...
| `apply/3` and reflection hide call sites from the checker | Confined to four allowlisted modules, each with an enumeration-driven test; the seam's check runs after compilation where it can see everything |
...
| Compiler options do not propagate from dependencies | `infer_signatures: true` in every app; documented for adopters in core's README, since the adoption guide is deferred |
