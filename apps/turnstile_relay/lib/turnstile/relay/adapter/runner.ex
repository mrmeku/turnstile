defmodule Turnstile.Relay.Adapter.Runner do
  @moduledoc false
  ...
  # pass and asks the arithmetic in `Turnstile.Relay.Core.Backoff` when the
  ...
  # otherwise, so `Turnstile.Relay.wake/1` takes that name, and a test starts
  # one unregistered and wakes it by pid.

  use GenServer

  alias Turnstile.Relay.Adapter.Drain
  alias Turnstile.Relay.Core.Backoff
  alias Turnstile.Relay.Options
  ...
  @doc "Start a runner. Options: `Turnstile.Relay.options_schema/0`."
