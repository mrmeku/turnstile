defmodule Turnstile.Relay.MixProject do
  use Mix.Project
  ...
  # The job the suite proves the template over, the rows it writes, and the
  # migrations that raise the tables both use are test support.
  ...
  # This package carries no authorization concept, so it takes none of the
  # library's packages in `lib`. The test run takes `turnstile` for the two
  # pieces of test support that are published for this purpose, the ephemeral
  # cluster and the repo seam its test repos are mediated by, and
  # `stream_data` for the durability property. ecto_sql is unrestricted
  # because the migration helper ships in lib, and postgrex because an
  # adopter's repo is a Postgres one and the advisory lock is Postgres's.
  # Every pin is exact, and each is the pin the umbrella already locks,
  ...
  defp deps do
    [
      ...
      {:turnstile, in_umbrella: true, only: :test},
