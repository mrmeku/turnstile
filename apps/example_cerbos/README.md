...
- `ExampleCerbos.Application`: boots `Turnstile.Config` with the sidecar's address and the ledger mode from the application env, binds `Example.Repo`, the declarations, the policy directory, and the commit that directory is at, starts the repos and the consumer of the events, and publishes the commit as a policy version.
