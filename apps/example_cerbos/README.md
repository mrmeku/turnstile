...
`Turnstile.Cerbos` adapter. This application is thin: it boots the
configuration, binds the attribute declarations and the policy directory to
the example's repo, and carries the migrations. What each rule of the
example is enforced by is the table below. The domain, its contexts, its web
...
- `config/test.exs`: the commit is pinned, because a scenario sets the commit a decision was made under beside the one a tightened policy is published as.
