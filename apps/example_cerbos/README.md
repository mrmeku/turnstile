...
- `config/config.exs`: the sidecar's address, the policy directory, and `POLICY_COMMIT`, the commit every decision under this binding names. `config/test.exs` pins the commit, because a scenario sets the commit a decision was made under beside the one a tightened policy is published as.
