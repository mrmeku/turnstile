...
`Turnstile.Postgres` adapter. This application is thin: it boots the
configuration, binds the example's schemas to the example's repo, and
carries the migrations that write the policies. What each rule of the
...
example; it is a property of this binding, and the table above records the database as what enforces C7 here.
