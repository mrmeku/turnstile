defmodule Turnstile.EdgeTest do
  ...
  test "convert reads each field of the spec, by atom or string key, in order" do
    ...
    assert {:error, %Error{reason: :invalid, detail: "invalid thing: missing note"}} = Edge.convert(map, spec, :thing)

    assert {:error, %Error{reason: :invalid, detail: detail}} =
             Edge.convert(%{map | "kind" => "c"}, spec, :thing, [:note])

    assert detail =~ "expected one of [:a, :b]"
  end

  test "value_in refuses nil for a plain string or integer and keeps it when allowed" do
    assert {:error, %Error{reason: :invalid}} = Edge.value_in(nil, :string, :thing)
    assert {:error, %Error{reason: :invalid}} = Edge.value_in(nil, :integer, :thing)
    ...
    assert {:error, %Error{reason: :invalid}} = Edge.value_in("3", :integer, :thing)
  ...
  test "atoms and modules read back only when they exist, from either form" do
    ...
    assert {:error, %Error{reason: :invalid, detail: "invalid thing: unknown atom " <> _rest}} =
             Edge.value_in("no-such-atom-xyz", :atom, :thing)

    assert {:error, %Error{reason: :invalid}} = Edge.value_in(1, :atom, :thing)
    ...
    assert {:error, %Error{reason: :invalid}} = Edge.value_in(1, :module, :thing)
  ...
  test "references and times read back from their map and ISO 8601 forms" do
    ...
    assert {:error, %Error{reason: :invalid, detail: "invalid thing: a reference needs" <> _rest}} =
             Edge.value_in(%{type: "folder"}, :ref, :thing)

    assert {:error, %Error{reason: :invalid}} = Edge.value_in(%{type: "no-such-atom-xyz", id: 1}, :ref, :thing)
    assert {:error, %Error{reason: :invalid}} = Edge.value_in("folder", :ref, :thing)
    ...
    assert {:error, %Error{reason: :invalid, detail: "invalid thing: bad time" <> _rest}} =
             Edge.value_in("yesterday", :time, :thing)

    assert {:error, %Error{reason: :invalid}} = Edge.value_in(1, :time, :thing)
  ...
  test "fetch_all names the first missing key" do
    ...
    assert {:error, %Error{reason: :invalid, detail: "invalid thing: missing c"}} =
             Edge.fetch_all(%{a: 1}, [:a, :c], :thing)
  end
end
