...
- `ExampleFga.Infrastructure.Guard`: the re-authentication window, read from the environment before the server is asked, for a fact about the session that no tuple should carry.
...
Two of the modules here are held to templates the adapter ships, over a population of the example's own rows. `ExampleFga.Infrastructure.TupleMappingTest` holds the mapping to the four things a pass relies on and asks nothing of a server. `ExampleFga.OutboxTest` holds the markers, the mapping, and a store together: what the tables say is what the store holds, and it stays that way.
