defmodule ExampleRbac.Core.Predicates do
  ...
  @spec controls(Turnstile.subject(), Turnstile.environment()) :: Ecto.Query.dynamic_expr()
  ...
  @spec portion_controls(Turnstile.subject(), Turnstile.environment()) :: Ecto.Query.dynamic_expr()
  ...
  @spec session(Turnstile.subject(), Turnstile.environment()) :: boolean()
  def session({_kind, _account}, %{now: _now} = environment), do: Sessions.fresh?(environment)

  @doc "C9: the approver is not the proposer."
  @spec another_approver(Turnstile.subject(), Turnstile.environment()) :: Ecto.Query.dynamic_expr()
