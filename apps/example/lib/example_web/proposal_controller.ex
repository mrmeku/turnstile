defmodule ExampleWeb.ProposalController do
  ...
  def create(conn, %{"id" => id} = params) do
    ...
    case Proposals.propose(conn.assigns.subject, String.to_integer(id), attrs, env: conn.assigns.env) do
  ...
  def approve(conn, %{"id" => id}) do
    case Proposals.approve(conn.assigns.subject, String.to_integer(id), env: conn.assigns.env) do
      {:ok, proposal} -> json(conn, Banner.marking(proposal.document.marking))
      {:error, _refusal} -> forbidden(conn)
    end
  end
