...
- `Example.Controls`: the four control kinds and the banner over a set of markings, which admits no subject any of them denies.
- `Example.Siem`: the consumer of the library's two events. Each change event and each decision event becomes an OCSF record of schema version 1.3.0, held in memory. `Example.Siem.Ocsf` carries the class, the activity, the severity, and the product metadata, which are the consumer's to choose because they move between schema versions; the event payload carries the rest.
...
- `test/support`: `Example.Fixture`, the world every scenario starts from; `Example.Scenarios`, every scenario of `docs/reference.md` §3a, declared once and defined in a thin application's test module with `use Example.Scenarios, capabilities: ThinApp.Capabilities, rules: ThinApp.Rules`, with the count test of `docs/testing.md` §6; `Example.Scenarios.Rules`, the policy operations and the policy-version event a thin application supplies.

`glossary.md` defines the words this package owns. The package's own tests cover the contexts and the web layer under `Turnstile.Test.Fake`; the scenarios run in each thin application, under the adapter it binds, and never here.
